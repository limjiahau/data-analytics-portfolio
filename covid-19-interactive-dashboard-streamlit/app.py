...
vacci_data = pd.read_csv('data/Ohio State Vaccination Summary.csv', thousands='', encoding='utf-8')
vacci_data['value'] = pd.to_numeric(vacci_data['value'])

vacci_rate = str(round(vacci_data.at[0,'value'] / (vacci_data.at[0,'value'] + vacci_data.at[1,'value']),2))
...
with osu_panel:
    ...
    with columns[2]:
        ...
        donut_chart = alt.Chart(vacci_data).mark_arc().encode(
