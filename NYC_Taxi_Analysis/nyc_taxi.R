...
# Most people travel as 1 passenger
# Friday was the most popular day of the week for total number of pickups 
# 18:00hrs was the most popular time of the day for total number of pickups 
multiplot(p1, p2, p3, p4, p5, layout=layout)


p1 <- 1; p2 <- 1; p3 <- 1; p4 <- 1; p5 <- 1

# Check number of passengers usually in a taxi
...
# Check vendor-wise trips and the patterns for different weekdays, hours of the day, months, locations, trip duration.
...
p2 <- train %>%
  ...
  labs(x = "Hour of the day", y = "count")
layout <- matrix(c(1,2),2,1,byrow=FALSE)
# plot for "count" and "Hour of the day" is generally consistent throughout the months
# plot for "count" and "Hour of the day" shows different results for weekends and weekdays
multiplot(p1, p2, layout=layout)
...
p2 <- train %>%
  ...
  theme(legend.position = "none")
layout <- matrix(c(1,2),2,1,byrow=FALSE)
# Thursday has the longest trip duration while Sunday has the shortest
multiplot(p1, p2, layout=layout)
...
train %>%
  ...
  # "Number of passengers" does not have much impact on "Trip duration"
...
#' ## Store and Forward vs "vendor_id"
#' 
train %>%
...
#'derived from the "pickup_datetime". 
#'#' From the coordinates of the pickup and dropoff points calculate 
#'the direct "distance" between the two points, and 
#'compare it to our "trip_duration", using the "distCosine" function of the [geosphere].
...
# lower speed during the weekdays likely due to increase traffic 
...
layout <- matrix(c(1,2,3,4),2,2,byrow=FALSE)
# find out why so few people are taking taxis in particular area
multiplot(p1, p2, p3, p4, layout=layout)
...
p1 + ggtitle("Longer than 24 hour trips for Manhattan")
p1 <- 1
...
p1 + ggtitle("Day-long trips for Manhattan")
p1 <- 1
...
p1 + ggtitle("Minute-long trips for Manhattan")
p1 <- 1
